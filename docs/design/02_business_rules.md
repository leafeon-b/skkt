...
- 研究会を削除した場合、研究会配下のデータは**物理削除（カスケード）**される
- 削除対象には以下を含む
  - セッション
  - 対局結果
  - 対局結果の編集履歴
  - メンバーシップ（論理削除済みレコードを含む）
...
- 研究会メンバーの脱退・除名は**論理削除**（`deletedAt` の設定）として扱う
- 研究会メンバーシップを論理削除しても、セッションメンバーシップはカスケード削除しない（認可チェックは研究会メンバーシップの `deletedAt: null` で担保）
- 再加入時は新規レコードを作成する（既存の論理削除レコードは復元しない）
- CircleOwner は脱退・除名できない（先にオーナー移譲が必要）
- メンバー数のカウントはアクティブなメンバー（`deletedAt IS NULL`）のみを対象とする
...
- 参加取消は**論理削除**（`deletedAt` の設定）として扱う
- 対局記録がある参加者も参加取消（論理削除）が可能
- 参加者数のカウントはアクティブな参加者（`deletedAt IS NULL`）のみを対象とする
...
- 対局結果を論理削除しても、セッション内の順序は再利用しないため欠番が発生し得る
