...
- 研究会メンバーの退会・除名後も、そのメンバーの記録は保持される
- 研究会を退会・除名されても、セッションの参加状態や権限には影響しない
- 退会・除名後の再加入は可能である
- CircleOwner は退会・除名できない（先にオーナー移譲が必要）
