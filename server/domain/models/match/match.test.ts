...
describe("Match ドメイン", () => {
  ...
  test("updateMatchPlayers は対局者を正しく更新する", () => {
    ...
    expect(updated.player1Id).toBe(toUserId("user-3"));
    expect(updated.player2Id).toBe(toUserId("user-4"));
  });
  ...
  test("updateMatchPlayers は対局者の違いを検証する", () => {
    ...
    expect(() =>
      updateMatchPlayers(match, toUserId("user-1"), toUserId("user-1")),
    ).toThrow("players must be different");
  });
  ...
  test("hasDifferentPlayers は異なるプレイヤーで true を返す", () => {
    expect(hasDifferentPlayers(toUserId("user-1"), toUserId("user-2"))).toBe(true);
  });

  test("hasDifferentPlayers は同一プレイヤーで false を返す", () => {
    expect(hasDifferentPlayers(toUserId("user-1"), toUserId("user-1"))).toBe(false);
  });
});
