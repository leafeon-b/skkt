...
describe("CircleSession セッションメンバーシップサービス", () => {
  ...
  test("listMemberships は一覧を返す", async () => {
    ...
    expect(result[0].circleSessionId).toBe(toCircleSessionId("session-1"));
  ...
  test("addMembership は研究会非メンバーかつ過去セッション参加者の再追加が成功する", async () => {
    ...
    const result = await service.addMembership({
      actorId: "user-actor",
      circleSessionId: toCircleSessionId("session-1"),
      userId: toUserId("non-circle-member"),
      role: "CircleSessionMember",
    });
  ...
  test("addMembership は研究会非メンバーかつ過去セッション参加歴なしの追加を拒否する", async () => {
    ...
    await expectReject(
      service.addMembership({
        actorId: "user-actor",
        circleSessionId: toCircleSessionId("session-1"),
        userId: toUserId("completely-new-user"),
        role: "CircleSessionMember",
      }),
      BadRequestError,
  ...
  describe("countPastSessionsByUserId", () => {
    ...
    test("参加データ0件 → 0を返す", async () => {
      const result = await service.countPastSessionsByUserId(toUserId("user-1"));
    ...
    test("全セッションが過去 → 全件カウント", async () => {
      ...
      await circleSessionRepository.addMembership(
        toCircleSessionId("past-1"),
        toUserId("user-1"),
        "CircleSessionMember",
      );
      await circleSessionRepository.addMembership(
        toCircleSessionId("past-2"),
        toUserId("user-1"),
        "CircleSessionMember",
      );
    ...
    test("全セッションが未来 → 0を返す", async () => {
      ...
      await circleSessionRepository.addMembership(
        toCircleSessionId("future-1"),
        toUserId("user-1"),
        "CircleSessionMember",
      );
      await circleSessionRepository.addMembership(
        toCircleSessionId("future-2"),
        toUserId("user-1"),
        "CircleSessionMember",
      );
