...
export default function CircleHeroClient() {
  ...
  const participationsQuery = trpc.circles.participations.list.useQuery(
    { circleId: circleId ?? "" },
    { enabled: Boolean(circleId) },
  );
  ...
  const participationCount = participationsQuery.data?.length;
  const subtitle = participationsQuery.isLoading
    ? "参加者を読み込み中..."
    : participationCount != null
      ? `参加者 ${participationCount}名 / 毎週土曜 18:00 - 21:00`
      : "参加者情報を取得できませんでした";
