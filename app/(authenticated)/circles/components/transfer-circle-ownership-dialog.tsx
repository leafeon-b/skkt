...
type TransferCircleOwnershipDialogProps = {
  circleId: string;
  viewerUserId: string;
  members: CircleOverviewMember[];
};

export function TransferCircleOwnershipDialog({
  circleId,
  viewerUserId,
  members,
}: TransferCircleOwnershipDialogProps) {
  ...
  const transfer = trpc.circles.memberships.transferOwnership.useMutation({
    onSuccess: () => {
  ...
  const selectedMember = candidates.find((m) => m.userId === selectedUserId);
