...
type RemoveCircleMemberButtonProps = {
  circleId: string;
  userId: string;
  memberName: string;
};

export function RemoveCircleMemberButton({
  circleId,
  userId,
  memberName,
}: RemoveCircleMemberButtonProps) {
  const [open, setOpen] = useState(false);
  const router = useRouter();

  const remove = trpc.circles.memberships.remove.useMutation({
    onSuccess: () => {
      setOpen(false);
      router.refresh();
  ...
  return (
    ...
    >
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>研究会から除外</AlertDialogTitle>
          <AlertDialogDescription>
            {memberName}をこの研究会から除外しますか？
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
